...
UPDATE Language_en_US
SET Text = 'The {TXT_KEY_UNIT_AMERICAN_MINUTEMAN} is very powerful as it requires 50% less expereince to unlock Range Promotion, thanks to Barrage I. Combined with ignoring tough terrain you can position many Minuteman quickly and strike cities or units many times being safe from ranged retaliation. Make sure to finish off units with them to gain Golden Age Points.{TXT_KEY_UNIT_AMERICAN_MINUTEMAN} are also excellent in defense as they can retreat in rough terrain and fire in the same turn.[NEWLINE][NEWLINE] Barrage Promotion Line is heavily recommeded thanks to the easy access to Range Promotion. Make Minutemen the core of your army at the cost of Melee and Siege units.'
...
UPDATE Language_en_US
SET Text = 'The main bonus of the {TXT_KEY_CIV5_BABYLON_WALLS} really game is the +3 Science, which is more than Library, so it is worth building {TXT_KEY_CIV5_BABYLON_WALLS} quickly.[NEWLINE][NEWLINE]In the late game the main benefit shifts to the 5% increase in Science generated by Great Scientists when used to research new Technologies. It because expenidng great Scienctists becomes much more valualbe than creating Academies in later stages of the game. This bonus alone is so strong that you cuould settle or conquer many cities throughout the game purely for this bonus, so that in the late game you may focus on creating Great Scientists and research technologies with them at an impressive rate. The significant boost to city defense is invaluable in frontier cities, which synergizes with the aggressive settling strategy as these vulnerable cities are easier to defend, especially with your Unique Units.'
...
UPDATE Language_en_US
SET Text = 'Thanks to the Blitz promotion {TXT_KEY_UNIT_AMAZONAS} not only can attack twice per turn, but as a side effect can move after attacking. This enables focus attacks on ships, which is strong as it allows to defeat enemy units more reliably. This synergises with {TXT_KEY_UNIT_AMAZONAS} gain of Golden Age Points and Great Admiral Points upon defeating enemy units which further synergises with Brazils Golden Age Unique Ability. However, Blitz also allows to attack cities with many ships in a single turn, which stopped being possible with the obsolecence of Galleases. Consider building many {TXT_KEY_UNIT_AMAZONAS} and performing mass sieges on cities with as many of them as Possible, even without siege promotions from Hull Promotion line.'
WHERE Tag = 'TXT_KEY_UNIT_AMAZONAS_STRATEGY';

UPDATE Language_en_US
SET Text = 'When aiming for Cultural Victory {TXT_KEY_BUILDING_EMBRAPA} is a must due to 5% Food to Tourism conversion. Tourism is a yield that is hard to come by, so focusing more of Food generation might be beneficial to take full advantage of this conversion process. Research Plastics technology early to unlock this building.[NEWLINE][NEWLINE]When not aiming for Cultural or Science victory it is still worth building {TXT_KEY_BUILDING_EMBRAPA} due to strong bonuses to Science which are always valuable. Especially in cities many Forest or Jungle tiles and Farm resources, as these gain significant yields. In Cities without these tiles {TXT_KEY_BUILDING_EMBRAPA} loses on significance so you may delay its construction.[NEWLINE][NEWLINE] In any case, working all the tiles strengthened by {TXT_KEY_BUILDING_EMBRAPA} is highly recommended.'
WHERE Tag = 'TXT_KEY_BUILDING_EMBRAPA_STRATEGY';
...
UPDATE Language_en_US
SET Text = 'The {TXT_KEY_UNIT_CARTHAGINIAN_QUINQUEREME} has arguably strongest promotions ships of Classical or Medieval era get, so . With Pincer, make sure to surround enemy ships as much has possible to exploit your enhanced flanking bonus. It should be easy thanks to ingnoring of the Zone of Control. Combined with higher combat strength and +25% bonus when attacking, enemy ships should not be a challenge. Likewise, attacking cities, especially before they build Castle is incentivised thanks to +50% bonus when attacking them. It is recommended to aim for the Boarding Party Promotion line, to reach Blitz Promotion as it allows moving after attack and hence attacking a cities with many ships in a single turn. ALternatively Hull promotions leading to Vanguard are also a good choice.'
WHERE Tag = 'TXT_KEY_UNIT_CARTHAGINIAN_QUINQUEREME_STRATEGY';
UPDATE Language_en_US

SET Text = 'Atlas Elephant should be the main force of your early army, because its as strong as Swordsman, but avialable earlier, requires no strategic resources and is faster. It is one of the strongest Classical Era units, and since its available at Trade, you can delay researching military technologies in favor of Trade, Writing and Sailing to focus on economic development of your cities. The ability to cross mountains allows for creative surprise attacks, so try to fight near mountain chains. Because this ability and +20hp bonus stays on upgrade you can maintain strong mounted army to later stages of the game. However, you should claim many horse resoures in advance, to be able to upgrade to Knights and Lancers.'
WHERE Tag = 'TXT_KEY_UNIT_CARTHAGINIAN_FOREST_ELEPHANT_STRATEGY';

SET Text = 'The {TXT_KEY_UNIT_ASAMU} allows for more aggressive settling, but requires very high Gold generation, as it can not be produced but only be purchased for a lot of Gold. Because purchasing one does not decrease population like producing a Settler does, this major bottleneck on the speed of settling early game is replaced by gold generation. Therefore, consider gathering as much gold as possible early game from your Unique Ability, destroying Barbarian Camps, exploring Ruins and working Gold tiles to then settle at a high pace. Because {TXT_KEY_UNIT_ASAMU} can retreat from melee attack once per turn, you can travel though barbarain terriory without escort, unless two units can attack it. Additionally, since {TXT_KEY_UNIT_ASAMU} can Embark without researching Sailing technology, you can claim valuable islands before your rivals. You can couple that with producing Settlers at the same time. This way you can secure many great city locations with many Luxury Resources, synersising with your Unique Ability.'
...
UPDATE Language_en_US
SET Text = 'The {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} is very powerful when fighting melee units, so use it primarily for this. Move it adjacent to enemy units you plan to attack with other units so that the Feared Elephant Promotion weakens the target for your other units. Make sure to stack your Great Generals with {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} to use the Combat Strength bonus.[NEWLINE][NEWLINE] Because it does not require horse resources you can build as many of them, so {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} should replace all of your melee units because they are weaker in most scenarios. While 3 movement is lower than standard 4 movement of mounted units, it still allows for good flexibility and focus attacks especially with the ability to move after attacking. You can use your unused horse resources to build skirmishers and flank enemy units before attacking with the {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} to increase the flanking bonus massively. Therefore Shock promotion line is heavily encouraged for {TXT_KEY_UNIT_SIAMESE_WARELEPHANT}. Mobility promotion becomes more valuable due to the lower movement speed of {TXT_KEY_UNIT_SIAMESE_WARELEPHANT}. In the future you can upgrade all of them to landships while having very few horse resources, as only the intermediate Lancer unit will require horses.'
WHERE Tag = 'TXT_KEY_UNIT_SIAMESE_WARELEPHANT_STRATEGY';

UPDATE Language_en_US
SET Text ='{TXT_KEY_UNIT_SUEA_MOP} lacks the bonus to combat strength when attacking cities, making it similar to a normal ranged unit. However it deals an extra 15 damage vs units or cities if it did not move before attacking in a turn. This ignores damage reduction and high combat strength of attacked units, so it effectively makes {TXT_KEY_UNIT_SUEA_MOP} just as effective when attacking cities and very effective when fighting units. The Naval Artillery Promotion is powerful vs ships because it both increases damage dealt to and reduces damage received from ships.[NEWLINE][NEWLINE]Use {TXT_KEY_UNIT_SUEA_MOP} as coastal defense or as a a ranged unit fighting land units or cities. It is poweful enough to disaplce your ranged units. Make sure you prtect it from melee attacks because it has low Comabt Strength and because it should avoid moving before attacking.[NEWLINE][NEWLINE]Logistics Promotion has the strongest synergy with {TXT_KEY_UNIT_SUEA_MOP} because you can apply the 15 damage twice per turn or you can move after attacking which will not affect this bonus next turn. Alternatively, Range Promotion will allow you to be stationary with more safety giving more reliability in dealing the bonus damage'
WHERE Tag = 'TXT_KEY_UNIT_SIAM_SUEA_MOP_STRATEGY';
